  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (state === "register") {
        ...
        login(token, refreshToken, { email: data.email });
    ...
    <div className="w-full h-[100vh] flex justify-center items-center bg-gray-900">
      ...
      >
        ...
        <div className="mt-5 text-left">
          <span className="text-sm text-indigo-500 dark:text-indigo-400 cursor-default">
            Forgot password?
          </span>
        </div>
        ...
        <button
          type="submit"
          className="mt-2 w-full h-11 rounded-full text-white bg-indigo-500 hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-60"
          disabled={loading}
        >
          {loading
            ? state === "login"
